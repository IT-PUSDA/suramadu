<?php
// Transaksi Surat Keluar - Nota Dinas (terpisah per jenis)
if (empty($_SESSION['admin'])) { $_SESSION['err'] = '<center>Anda harus login terlebih dahulu!</center>'; header('Location: index.php'); die(); }

$id_user = $_SESSION['id_user'];

// Sub-pages (add/edit/delete/process) tetap reuse file yang sama
if (isset($_REQUEST['sub'])) {
    switch ($_REQUEST['sub']) {
        case 'add_nota_dinas': include 'tambah_surat_keluar_nota_dinas.php'; break;
        case 'edit': include 'edit_surat_keluar.php'; break;
        case 'del': include 'hapus_surat_keluar.php'; break;
        case 'proses_tambah_nota_dinas': include 'proses_tambah_surat_keluar_nota_dinas.php'; break;
        default: include 'tambah_surat_keluar_nota_dinas.php'; break;
    }
...
// Cek apakah kolom jenis tersedia untuk dipakai sebagai filter
$hasJenis = false; $resJenis = mysqli_query($config, "SHOW COLUMNS FROM tbl_surat_keluar LIKE 'jenis'");
if ($resJenis && mysqli_num_rows($resJenis) === 1) { $hasJenis = true; } else {
  // Coba buat kolom 'jenis' otomatis jika belum ada
  mysqli_query($config, "ALTER TABLE tbl_surat_keluar ADD COLUMN jenis VARCHAR(20) NOT NULL DEFAULT 'umum'");
  $chk = mysqli_query($config, "SHOW COLUMNS FROM tbl_surat_keluar LIKE 'jenis'");
  if ($chk && mysqli_num_rows($chk) === 1) { $hasJenis = true; }
}
...
<script>
// Delegated copy-to-clipboard handler for nomor surat (inline bubble)
document.addEventListener('click', function (e) {
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(nomor).then(function(){ showCopyBubble(btn,'Nomor telah disalin'); }).catch(function(){ fallbackCopy(nomor); });
        ...
        <div class="z-depth-1">
            <nav class="secondary-nav">
                <div class="nav-wrapper blue-grey darken-1">
                    <div class="col m7">
                        <ul class="left">
                            <li class="waves-effect waves-light hide-on-small-only"><a href="index.php?page=admin&act=tsk_nd" class="judul"><i class="material-icons">drafts</i> Surat Keluar - Nota Dinas</a></li>
                            <li class="waves-effect waves-light"><a href="index.php?page=admin&act=tsk_nd&sub=add_nota_dinas"><i class="material-icons md-24">add_circle</i> Tambah Data</a></li>
                        </ul>
                    </div>
                    <div class="col m5 hide-on-med-and-down">
                        <form method="post" action="index.php?page=admin&act=tsk_nd">
...
<?php
// Notif
foreach (['succAdd','succEdit','succDel'] as $k) {
  ...
  <div class="col m12" id="colres">
    <div class="card">
      <div class="card-content">
        <?php if (isset($_REQUEST['submit'])) { $cari = mysqli_real_escape_string($config, $_REQUEST['cari']); echo '<div class="card-panel blue-grey lighten-5" style="margin-bottom: 20px;"><p class="blue-grey-text">Hasil pencarian untuk: <strong class="black-text">'.stripslashes($cari).'</strong></p></div>'; } ?>
              ...
              <?php if (mysqli_num_rows($query) > 0) { $seq = $curr + 1; while ($row = mysqli_fetch_array($query)) { ?>
                <tr style="vertical-align: top;">
                  ...
                  <td class="center-align">
                    ...
                    else { $can_manage = in_array($_SESSION['admin'], [1]); $is_owner = ($row['id_user'] == $_SESSION['id_user']); $is_operator_owner = $is_operator && !empty($operator_allowed_ids) && in_array((int)$row['id_user'],$operator_allowed_ids,true); if ($can_manage || $is_owner || $is_operator_owner) { echo '<div class="actions-compact" style="display:flex;justify-content:center;flex-wrap:wrap;padding-top:2px;">'; $btnBase='data-position="top"'; if(empty($__printedActionStyleND)){ echo '<style>.action-round{background:#1976d2;border-radius:50%;width:44px;height:44px;display:inline-flex;align-items:center;justify-content:center;box-shadow:0 2px 4px rgba(0,0,0,.15);transition:.25s;margin:3px 4px} .action-round i{line-height:44px;font-size:22px;color:#fff} .action-round.delete{background:#e64a19} .action-round.toggle{background:#2e7d32} .action-round.arch{background:#fbc02d!important} .action-round.arch.done{background:#9e9e9e!important} .action-round:hover{filter:brightness(1.08);} .action-round:active{transform:scale(.92);} @media(max-width:600px){.action-round{width:40px;height:40px;} .action-round i{font-size:20px;line-height:40px;}}</style>'; $__printedActionStyleND=true; } $is_operator_level = ($_SESSION['admin']==3); if($is_operator_level){ $toggleTitle = ($status_raw=='finished')?'Set Draft':'Set Finished'; echo '<a class="waves-effect waves-light tooltipped action-round toggle" '.$btnBase.' data-tooltip="'.$toggleTitle.'" href="#" onclick="return toggleStatus('.$row['id_surat'].', event);"><i class="material-icons">autorenew</i></a>'; } if ($_SESSION['admin']==1 || $is_operator_owner) { echo '<a class="waves-effect waves-light tooltipped action-round" '.$btnBase.' data-tooltip="Edit" href="?page=admin&act=tsk_nd&sub=edit&id_surat='.$row['id_surat'].'"><i class="material-icons">edit</i></a>'; echo '<a class="waves-effect waves-light tooltipped action-round delete" '.$btnBase.' data-tooltip="Hapus" href="?page=admin&act=tsk_nd&sub=del&id_surat='.$row['id_surat'].'" onclick="return confirm(\'Yakin ingin menghapus surat ini?\');"><i class="material-icons">delete</i></a>'; if($is_operator_level){ $alreadyArsip=!empty($row['id_arsip_berkas']); $archTooltip=$alreadyArsip?'Sudah diarsipkan':'Arsipkan'; $archCls=$alreadyArsip?' arch done':' arch'; $archOnclick=$alreadyArsip?'return false;':'return openArsipModal('.$row['id_surat'].');'; echo '<a class="waves-effect waves-light tooltipped action-round'.$archCls.'" '.$btnBase.' data-tooltip="'.$archTooltip.'" href="#" onclick="'.$archOnclick.'"><i class="material-icons">archive</i></a>'; } } else { echo '<a class="waves-effect waves-light tooltipped action-round pin-trigger" '.$btnBase.' data-tooltip="Edit (PIN)" href="?page=admin&act=tsk_nd&sub=edit&id_surat='.$row['id_surat'].'" data-action-type="edit" data-id-surat="'.$row['id_surat'].'"><i class="material-icons">edit</i></a>'; echo '<a class="waves-effect waves-light tooltipped action-round delete pin-trigger" '.$btnBase.' data-tooltip="Hapus (PIN)" href="?page=admin&act=tsk_nd&sub=del&id_surat='.$row['id_surat'].'" data-action-type="delete" data-id-surat="'.$row['id_surat'].'"><i class="material-icons">delete</i></a>'; if($is_operator_level){ $alreadyArsip=!empty($row['id_arsip_berkas']); $archTooltip=$alreadyArsip?'Sudah diarsipkan':'Arsipkan'; $archCls=$alreadyArsip?' arch done':' arch'; $archOnclick=$alreadyArsip?'return false;':'return openArsipModal('.$row['id_surat'].');'; echo '<a class="waves-effect waves-light tooltipped action-round'.$archCls.'" '.$btnBase.' data-tooltip="'.$archTooltip.'" href="#" onclick="'.$archOnclick.'"><i class="material-icons">archive</i></a>'; } } echo '</div>'; } else { echo '<div class="grey-text" style="padding-top: 15px;">-</div>'; } } ?>
                  </td>
                </tr>
...
$cdata = mysqli_num_rows($query_pg); $cpg = ceil($cdata / $limit);
$extra = '';
if (!empty($_GET['filter_bidang'])) { $extra .= '&filter_bidang=' . urlencode($_GET['filter_bidang']); }
echo '<br/><div class="center-align" style="margin:12px 0 8px;"><ul class="pagination pager">';
if ($cdata > $limit) {
    if ($pg > 1) { $prev = $pg - 1; echo '<li><a href="index.php?page=admin&act=tsk_nd&pg=1'.$extra.'"><i class="material-icons md-48">first_page</i></a></li><li><a href="index.php?page=admin&act=tsk_nd&pg='.$prev.$extra.'"><i class="material-icons md-48">chevron_left</i></a></li>'; } else { echo '<li class="disabled"><a><i class="material-icons md-48">first_page</i></a></li><li class="disabled"><a><i class="material-icons md-48">chevron_left</i></a></li>'; }
    if ($pg < $cpg) { $next = $pg + 1; echo '<li><a href="index.php?page=admin&act=tsk_nd&pg='.$next.$extra.'"><i class="material-icons md-48">chevron_right</i></a></li><li><a href="index.php?page=admin&act=tsk_nd&pg='.$cpg.$extra.'"><i class="material-icons md-48">last_page</i></a></li>'; } else { echo '<li class="disabled"><a><i class="material-icons md-48">chevron_right</i></a></li><li class="disabled"><a><i class="material-icons md-48">last_page</i></a></li>'; }
}
echo '</ul></div>';
?>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let targetUrl = '', actionType = '', suratId = '';

  function openModal(){ modal.style.display='flex'; setTimeout(()=>modal.classList.add('active'),10); pinInputs[0].focus(); }
  function closeModal(){ modal.classList.remove('active'); setTimeout(()=>{ modal.style.display='none'; resetModal(); },300); }
  function resetModal(){ pinForm.reset(); pinInputs.forEach(i=>i.value=''); errorMessage.textContent=''; targetUrl=''; actionType=''; suratId=''; }
  ...
  pinInputs.forEach((input, idx)=>{
    input.addEventListener('input', ()=>{ if(input.value && idx < pinInputs.length-1){ pinInputs[idx+1].focus(); } updateFullPin(); });
    input.addEventListener('keydown', (e)=>{ if(e.key==='Backspace' && !input.value && idx>0){ pinInputs[idx-1].focus(); }});
    input.addEventListener('paste', (e)=>{ e.preventDefault(); const t=(e.clipboardData||window.clipboardData).getData('text').replace(/\s/g,'').slice(0,6); t.split('').forEach((c,i)=>{ if(pinInputs[i]) pinInputs[i].value=c; }); const li=Math.min(t.length,6)-1; if(li>=0) pinInputs[li].focus(); updateFullPin(); });
  });

  pinForm.addEventListener('submit', function(e){
    e.preventDefault(); updateFullPin();
    if(fullPinInput.value.length !== 6){ errorMessage.textContent='PIN harus terdiri dari 6 digit.'; return; }
    errorMessage.textContent='';
    const formData = new FormData(); formData.append('id_surat', suratId); formData.append('pin', fullPinInput.value);
    fetch('src/Utils/verifikasi_pin_ajax.php', { method:'POST', body: formData })
      .then(r=>r.json()).then(data=>{
        if(data.success){ closeModal(); setTimeout(()=>{ if(actionType==='delete'){ if(confirm('PIN terverifikasi. Apakah Anda yakin ingin menghapus data ini?')){ window.location.href = targetUrl; } } else if(actionType==='view'){ window.open(targetUrl,'_blank'); } else { window.location.href = targetUrl; } }, 200); }
        else { errorMessage.textContent = data.message || 'PIN salah. Coba lagi.'; pinInputs.forEach(i=>i.value=''); pinInputs[0].focus(); }
      })
