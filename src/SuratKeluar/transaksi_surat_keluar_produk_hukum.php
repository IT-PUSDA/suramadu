<?php
// Transaksi Surat Keluar - Produk Hukum
...
if (isset($_REQUEST['sub'])) {
    switch ($_REQUEST['sub']) {
        case 'add_produk_hukum': include 'tambah_surat_keluar_produk_hukum.php'; break;
        case 'edit': include 'edit_surat_keluar.php'; break;
        case 'del': include 'hapus_surat_keluar.php'; break;
        case 'proses_tambah_produk_hukum': include 'proses_tambah_surat_keluar_produk_hukum.php'; break;
        default: include 'tambah_surat_keluar_produk_hukum.php'; break;
    }
    return;
}
        ...
        <div class="z-depth-1">
            <nav class="secondary-nav">
                <div class="nav-wrapper blue-grey darken-1">
                    <div class="col m7">
                        <ul class="left">
                            <li class="waves-effect waves-light hide-on-small-only"><a href="index.php?page=admin&act=tsk_ph" class="judul"><i class="material-icons">drafts</i> Surat Keluar - Produk Hukum</a></li>
                            <li class="waves-effect waves-light"><a href="index.php?page=admin&act=tsk_ph&sub=add_produk_hukum"><i class="material-icons md-24">add_circle</i> Tambah Data</a></li>
                        </ul>
                    </div>
                    <div class="col m5 hide-on-med-and-down">
                        <form method="post" action="index.php?page=admin&act=tsk_ph">
...
if ($hasJenis) { $where_clause .= " WHERE jenis='produk_hukum'"; }
// Scoping
          ...
          <table class="striped highlight responsive-table" id="tbl">
            ...
            <?php if (mysqli_num_rows($query)>0) { $seq=$curr+1; while ($row = mysqli_fetch_array($query)) { ?>
              <tr style="vertical-align: top;">
                ...
                <td class="center-align"><?php if ($_SESSION['admin']==2) { echo '<div class="grey-text" style="padding-top: 15px;">-</div>'; } else { $can_manage = in_array($_SESSION['admin'], [1]); $is_owner = ($row['id_user'] == $_SESSION['id_user']); $is_operator_owner = $is_operator && !empty($operator_allowed_ids) && in_array((int)$row['id_user'],$operator_allowed_ids,true); if ($can_manage || $is_owner || $is_operator_owner) { echo '<div class="actions-compact" style="display:flex;justify-content:center;flex-wrap:wrap;padding-top:2px;">'; $btnBase='data-position="top"'; if(empty($__printedActionStylePH)){ echo '<style>.action-round{background:#1976d2;border-radius:50%;width:44px;height:44px;display:inline-flex;align-items:center;justify-content:center;box-shadow:0 2px 4px rgba(0,0,0,.15);transition:.25s;margin:3px 4px} .action-round i{line-height:44px;font-size:22px;color:#fff} .action-round.delete{background:#e64a19} .action-round.toggle{background:#2e7d32} .action-round.arch{background:#fbc02d!important} .action-round.arch.done{background:#9e9e9e!important} .action-round:hover{filter:brightness(1.08);} .action-round:active{transform:scale(.92);} @media(max-width:600px){.action-round{width:40px;height:40px;} .action-round i{font-size:20px;line-height:40px;}}</style>'; $__printedActionStylePH=true; } $is_operator_level = ($_SESSION['admin']==3); if($is_operator_level){ $toggleTitle = ($status_raw=='finished')?'Set Draft':'Set Finished'; echo '<a class="waves-effect waves-light tooltipped action-round toggle" '.$btnBase.' data-tooltip="'.$toggleTitle.'" href="#" onclick="return toggleStatus('.$row['id_surat'].', event);"><i class="material-icons">autorenew</i></a>'; } if ($_SESSION['admin']==1 || $is_operator_owner) { echo '<a class="waves-effect waves-light tooltipped action-round" '.$btnBase.' data-tooltip="Edit" href="?page=admin&act=tsk_ph&sub=edit&id_surat='.$row['id_surat'].'"><i class="material-icons">edit</i></a>'; echo '<a class="waves-effect waves-light tooltipped action-round delete" '.$btnBase.' data-tooltip="Hapus" href="?page=admin&act=tsk_ph&sub=del&id_surat='.$row['id_surat'].'" onclick="return confirm(\'Yakin ingin menghapus surat ini?\');"><i class="material-icons">delete</i></a>'; if($is_operator_level){ $alreadyArsip=!empty($row['id_arsip_berkas']); $archTip=$alreadyArsip?'Sudah diarsipkan':'Arsipkan'; $archCls=$alreadyArsip?' arch done':' arch'; $archOnclick=$alreadyArsip?'return false;':'return openArsipModal('.$row['id_surat'].');'; echo '<a class="waves-effect waves-light tooltipped action-round'.$archCls.'" '.$btnBase.' data-tooltip="'.$archTip.'" href="#" onclick="'.$archOnclick.'"><i class="material-icons">archive</i></a>'; } } else { echo '<a class="waves-effect waves-light tooltipped action-round pin-trigger" '.$btnBase.' data-tooltip="Edit (PIN)" href="?page=admin&act=tsk_ph&sub=edit&id_surat='.$row['id_surat'].'" data-action-type="edit" data-id-surat="'.$row['id_surat'].'"><i class="material-icons">edit</i></a>'; echo '<a class="waves-effect waves-light tooltipped action-round delete pin-trigger" '.$btnBase.' data-tooltip="Hapus (PIN)" href="?page=admin&act=tsk_ph&sub=del&id_surat='.$row['id_surat'].'" data-action-type="delete" data-id-surat="'.$row['id_surat'].'"><i class="material-icons">delete</i></a>'; if($is_operator_level){ $alreadyArsip=!empty($row['id_arsip_berkas']); $archTip=$alreadyArsip?'Sudah diarsipkan':'Arsipkan'; $archCls=$alreadyArsip?' arch done':' arch'; $archOnclick=$alreadyArsip?'return false;':'return openArsipModal('.$row['id_surat'].');'; echo '<a class="waves-effect waves-light tooltipped action-round'.$archCls.'" '.$btnBase.' data-tooltip="'.$archTip.'" href="#" onclick="'.$archOnclick.'"><i class="material-icons">archive</i></a>'; } } echo '</div>'; } else { echo '<div class="grey-text" style="padding-top: 15px;">-</div>'; } } ?></td>
              </tr>
...
<script>
// Live search for Produk Hukum
document.addEventListener('DOMContentLoaded', function(){
  const searchInput = document.getElementById('search');
  const tbody = document.getElementById('tbody-data');
  if(!searchInput || !tbody) return;
  const originalHTML = tbody.innerHTML;
  const jenis = 'produk_hukum';
  const infoPanelId = 'search-info-panel';
  if(!document.getElementById(infoPanelId)){
    const panel = document.createElement('div');
    panel.id = infoPanelId; panel.className='card-panel blue-grey lighten-5'; panel.style.cssText='margin-bottom:20px;display:none';
    panel.innerHTML = '<p class="blue-grey-text">Hasil pencarian untuk: <strong class="black-text" id="search-info-text"></strong></p>';
    const cardContent = tbody.closest('.card-content');
    if(cardContent) cardContent.insertBefore(panel, cardContent.firstChild);
  }
  ...
  const doSearch = debounce(()=>{
    ...
    if(q===''){ tbody.innerHTML = originalHTML; updateInfo(''); rebindPins(); return; }
    updateInfo(q);
    fetch(`src/SuratKeluar/ajax_search_surat_keluar.php?jenis=${jenis}&cari=${encodeURIComponent(q)}${getFilterBidangParam()}`, {credentials:'same-origin'})
      .then(r=>{ if(!r.ok) throw new Error('HTTP '+r.status); return r.text(); })
      .then(html=>{ tbody.innerHTML=html; rebindPins(); })
      .catch(err=>console.error('Live search error (PH):', err));
  },300);
  searchInput.addEventListener('input', doSearch);
});
</script>

<?php $query_pg = mysqli_query($config, "SELECT 1 " . $base_query . $where_clause); $cdata = mysqli_num_rows($query_pg); $cpg = ceil($cdata / $limit); $extra=''; if (!empty($_GET['filter_bidang'])) { $extra .= '&filter_bidang=' . urlencode($_GET['filter_bidang']); } echo '<br/><div class="center-align" style="margin:12px 0 8px;"><ul class="pagination pager">'; if ($cdata>$limit) { if ($pg>1) { $prev=$pg-1; echo '<li><a href="index.php?page=admin&act=tsk_ph&pg=1'.$extra.'"><i class="material-icons md-48">first_page</i></a></li><li><a href="index.php?page=admin&act=tsk_ph&pg='.$prev.$extra.'"><i class="material-icons md-48">chevron_left</i></a></li>'; } else { echo '<li class="disabled"><a><i class="material-icons md-48">first_page</i></a></li><li class="disabled"><a><i class="material-icons md-48">chevron_left</i></a></li>'; } if ($pg<$cpg) { $next=$pg+1; echo '<li><a href="index.php?page=admin&act=tsk_ph&pg='.$next.$extra.'"><i class="material-icons md-48">chevron_right</i></a></li><li><a href="index.php?page=admin&act=tsk_ph&pg='.$cpg.$extra.'"><i class="material-icons md-48">last_page</i></a></li>'; } else { echo '<li class="disabled"><a><i class="material-icons md-48">chevron_right</i></a></li><li class="disabled"><a><i class="material-icons md-48">last_page</i></a></li>'; } } echo '</ul></div>'; ?>
