        ...
        if($_SESSION['admin'] != 1 AND $_SESSION['admin'] != 2){
            ...
            if(isset($_REQUEST['sub'])){
                ...
                if(isset($_REQUEST['submit'])){
                    ...
                    if ($_REQUEST['jenis_surat'] == "" || $_REQUEST['masa_aktif'] == ""){
                        ...
                        $masa_aktif = $_REQUEST['masa_aktif'];
                        ...
                        if(!preg_match("/^[a-zA-Z0-9. -]*$/", $nama)){
                            ...
                            if(!preg_match("/^[a-zA-Z0-9. -]*$/", $institusi)){
                                $_SESSION['institusi'] = 'Form Nama SKPD hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan minus(-)';
                                echo '<script language="javascript">window.history.back();</script>';
                            } else {
                                ...
                                if(!preg_match("/^[a-zA-Z0-9.,:()\/ -@]*$/", $alamat)){
                                    ...
                                    if(!preg_match("/^[a-zA-Z0-9.,:()\/ -@]*$/", $kota)){
                                        $_SESSION['kota'] = 'Form kota hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan kurung()';
                                        echo '<script language="javascript">window.history.back();</script>';
                                    } else {

                                        if(!preg_match("/^[a-zA-Z., ]*$/", $pimpinan)){
                                            $_SESSION['pimpinan'] = 'Form Nama Pimpinan hanya boleh mengandung karakter huruf, spasi, titik(.) dan koma(,)<br/><br/>';
                                            echo '<script language="javascript">window.history.back();</script>';
                                        } else {

                                            if(!preg_match("/^[0-9 -]*$/", $nip)){
                                                $_SESSION['nippimpinan'] = 'Form NIP Pimpinan hanya boleh mengandung karakter angka, spasi, dan minus(-)<br/><br/>';
                                                echo '<script language="javascript">window.history.back();</script>';
                                            } else {
                                                        ...
                                                        if(!empty($logo)){
                                                            ...
                                                            if(in_array($eks, $ekstensi) == true){
                                                                if($ukuran < 2000000){
                                                                    ...
                                                                    $query = mysqli_query($config, "UPDATE tbl_instansi SET institusi='$institusi',nama='$nama',alamat='$alamat',kota='$kota',pimpinan='$pimpinan',nip='$nip',website='$website',email='$email',logo='$nlogo',id_user='$id_user' WHERE id_instansi='$id_instansi'");
                                                            ...
                                                            $query = mysqli_query($config, "UPDATE tbl_instansi SET institusi='$institusi',nama='$nama',alamat='$alamat',kota='$kota',pimpinan='$pimpinan',nip='$nip',website='$website',email='$email',id_user='$id_user' WHERE id_instansi='$id_instansi'");
                    ...
                    $query = mysqli_query($config, "SELECT * FROM tbl_inaktif");
                            ...
                            <div class="col s12">
                                <nav class="secondary-nav">
                                    <div class="nav-wrapper blue-grey darken-1">
                                        <ul class="left">
                                            <li class="waves-effect waves-light"><a href="?page=sett" class="judul"><i class="material-icons">work</i> Manajemen Inaktif</a></li>
                                        </ul>
                                    </div>
                                </nav>
                            </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        <input id="institusi" type="text" class="validate" name="institusi" value="<?php echo $row['institusi']; ?>" required>
                                            ...
                                            ?>
                                        <label for="institusi">Nama SKPD</label>
                                    </div>
                                    <div class="input-field col s6">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        <input id="kota" type="text" class="validate" name="kota" value="<?php echo $row['kota']; ?>" required>
                                            <?php
                                                if(isset($_SESSION['kota'])){
                                                    $kota = $_SESSION['kota'];
                                                    echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$kota.'</div>';
                                                    unset($_SESSION['kota']);
                                                }
                                            ?>
                                        <label for="kota">Kota - Kode Pos</label>
                                    </div>
