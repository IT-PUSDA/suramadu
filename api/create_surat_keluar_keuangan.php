...
if (!(ctype_digit($raw_pin) && strlen($raw_pin) === 6)) {
    http_response_code(400); echo json_encode(['status' => 'error', 'message' => 'PIN harus berupa tepat 6 digit angka']); exit;
}
...
// Additional validation for KEUANGAN: Bidang must be allowed
if (function_exists('get_allowed_bidang_codes')) {
    $allowed = get_allowed_bidang_codes();
    if (!in_array($bidang, $allowed, true)) {
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Bidang code is not valid for Keuangan (must be in allowed list)']);
        exit;
    }
}

// --- Logic (derived from proses_tambah_surat_keluar_keuangan.php) ---
