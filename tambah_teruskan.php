<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($_REQUEST['id_tujuan_ter'] == "" || $_REQUEST['isi_ter'] == "" || $_REQUEST['sifat'] == "" || $_REQUEST['catatan'] == ""){
				...
				if($id_tujuan_ter>0)
				{
					$query1 = "SELECT * FROM tbl_user where id_user='$id_tujuan_ter' ";
                ...
                if(!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $tujuan_ter)){
                    $_SESSION['tujuan_terd'] = 'Form Tujuan Terusan hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,) minus(-). kurung() dan garis miring(/)';
                    echo '<script language="javascript">window.history.back();</script>';
                } else {

                    if(!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $isi_ter)){
                        $_SESSION['isi_terd'] = 'Form Isi Terusan hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan(&), underscore(_), kurung(), persen(%) dan at(@)';
                        echo '<script language="javascript">window.history.back();</script>';
                    } else {
                        ...
                        if(!preg_match("/^[a-zA-Z0 ]*$/", $sifat)){
                            ...
                            if(!preg_match("/^[a-zA-Z0-9.,()%@\/ -]*$/", $catatan)){
                                ...
                                echo '<script language="javascript">window.history.back();</script>';
                                    ...
                                    $query = mysqli_query($config, "INSERT INTO tbl_teruskan(id_teruskan,no_agenda,id_tujuan_terusan,tujuan_terusan,isi_terusan,sifat,tgl_terusan,
									catatan,id_surat,id_user) VALUES('$id_ter','$no_agenda','$id_tujuan_ter','$tujuan_ter','$isi_ter','$sifat',NOW(),'$catatan','$id_surat','$id_user')");
                                    if($query == true){
                ...
                <div class="col s12">
                    <nav class="secondary-nav">
                        <div class="nav-wrapper blue-grey darken-1">
                            <ul class="left">
                                <li class="waves-effect waves-light"><a href="#" class="judul"><i class="material-icons">description</i> Input Teruskan Surat</a></li>
                            </ul>
                        </div>
                    </nav>
                </div>
                ...
                <form class="col s12" method="post" action="">
                    ...
                    <!-- Row in form START -->
                        ...
                        <div class="input-field col s6">
							...
							<?php
									$q = mysqli_query($config, "SELECT max(id_teruskan) as idterbesar FROM tbl_teruskan ");
                                ...
                                ?>
							<input id="id_ter" type="text" class="validate" name="id_ter" value="<?php echo $id_ter; ?>" hidden required>
                        ...
                        <div class="input-field col s6">
                            ...
                            <textarea id="isi_ter" class="materialize-textarea validate" name="isi_ter" required></textarea>
                                <?php
                                    if(isset($_SESSION['isi_terd'])){
                                        $isi_ter = $_SESSION['isi_terd'];
                                        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">'.$isi_terd.'</div>';
                                        unset($_SESSION['isi_terd']);
                                    }
                                ?>
                            <label for="isi_terusan">Isi Terusan</label>
